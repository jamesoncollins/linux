...
static inline void axi_pwmgen_write_mask(struct axi_pwmgen *pwm,
					 unsigned int reg,
					 unsigned int mask,
					 unsigned int value)
{
			     ...
			     const struct pwm_state *state)
{
	...
	tmp = (clk_rate / NSEC_PER_USEC) * state->offset;
	offset_cnt = DIV_ROUND_UP(tmp, USEC_PER_SEC);
	/* The offset_cnt is the inverse of the actual time offset */
	axi_pwmgen_write(pwm, AXI_PWMGEN_CHX_OFFSET(ch),
		state->offset ? (period_cnt - offset_cnt) : 0);
...
static void axi_pwmgen_disable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	axi_pwmgen_write(pwmgen, AXI_PWMGEN_REG_CONFIG,
...
static int axi_pwmgen_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	axi_pwmgen_write(pwmgen, AXI_PWMGEN_CHX_PERIOD(ch),
			 pwmgen->ch_period[ch]);
	axi_pwmgen_write(pwmgen, AXI_PWMGEN_REG_CONFIG,
			 AXI_PWMGEN_LOAD_CONIG);
